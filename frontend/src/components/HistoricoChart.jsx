...
export default function HistoricoChart({ item }) {
  ...
  const config = useMemo(() => {
    ...
    if (item.tipo === 'simulacao') {
      ...
      if (r) {
        ...
        series = [
          Math.max(
            0,
            Number(r.impostoPj) || 0
          ),

          Math.max(
            0,
            Number(r.provisoesSimuladasPj) || 0
          ),

          Math.max(
            0,
            Number(r.margemDisponivel) || 0
          ),
        ];
    ...
    const taxaAA =
      (Number(p?.taxaRentabilidade) || 11.5) / 100;

    const inflacaoAA =
      (Number(p?.taxaInflacao) || 4.5) / 100;
